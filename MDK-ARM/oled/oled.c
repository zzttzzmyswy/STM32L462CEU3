...
{
    HAL_I2C_Mem_Write(&hi2c1, 0x78, 0x40, I2C_MEMADD_SIZE_8BIT, &data, 1,
                      0x100);
}
//初始化oled屏幕
void OLED_Init(void)
                  ...
                  uint8_t size2)
{
    ...
    for (t = 0; t < len; t++)
    {
        ...
        if (enshow == 0 && t < (len - 1))
        {
            if (temp == 0)
            {
                OLED_ShowChar(x + (size2 == BIG_FONT_EN ? 8 : 6) * t, y, ' ',
                              size2);
                continue;
            }
            else
        ...
        OLED_ShowChar(x + (size2 == BIG_FONT_EN ? 8 : 6) * t, y, temp + '0',
                      size2);
    }
}
//显示一个字符号串
void OLED_ShowString(uint8_t x, uint8_t y, uint8_t *chr, uint8_t Char_Size)
{
    ...
    while (chr[j] != '\0')
    {
        ...
        x += Char_Size == BIG_FONT_EN ? 8 : 7;
        if (x > (Char_Size == BIG_FONT_EN ? 120 : 121))
        {
            x = 0;
            y += Char_Size == BIG_FONT_EN ? 16 : 8;
        }
        j++;
    }
}
